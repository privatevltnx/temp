...
function loadAppDetails() {
    ...
    iconImg.onerror = function() {
        appIcon.src = 'https://via.placeholder.com/120?text=App';
    };
    iconImg.src = currentApp.icon || 'https://via.placeholder.com/120';
    ...
    if (currentApp.downloads && currentApp.downloads.length > 0) {
        currentApp.downloads.forEach(download => {
            ...
            if (download.type.includes('Tutorial')) {
                btn.innerHTML = `<i class="fas fa-book"></i> <span>${download.type}</span>`;
                btn.classList.add('tutorial-btn');
            } else {
                btn.innerHTML = `<i class="fas fa-download"></i> <span>${download.type} (${download.size})</span>`;
            }
...
function loadScreenshots() {
    ...
    if (!currentApp.screenshots || currentApp.screenshots.length === 0) {
        screenshotsContainer.innerHTML = '<p style="color: var(--text-secondary); grid-column: 1/-1;">No screenshots available</p>';
        return;
    }
    
    screenshotsContainer.innerHTML = currentApp.screenshots.map((screenshot, index) => 
        `<img src="${screenshot}" alt="Screenshot ${index + 1}" data-index="${index}" class="screenshot-img" loading="lazy">`
    ).join('');
    ...
    setTimeout(() => {
        document.querySelectorAll('.screenshot-img').forEach((img, index) => {
            img.addEventListener('click', function() {
                openScreenshotModal(currentApp.screenshots[index], index);
            });
            // Preload images
            img.onerror = function() {
                this.src = 'https://via.placeholder.com/200x350?text=Screenshot';
            };
        });
    }, 100);
}
...
function openScreenshotModal(imageSrc, index) {
    ...
    img.onerror = function() {
        modalImg.src = 'https://via.placeholder.com/400x600?text=Screenshot';
        counter.textContent = `${index + 1} / ${currentApp.screenshots.length}`;
        modal.style.display = 'flex';
        modal.currentIndex = index;
    };
...
function createScreenshotModal() {
    ...
    modal.innerHTML = `
        <div class="modal-content">
            ...
            <img class="modal-image" src="" alt="Screenshot" loading="eager">
            <div class="modal-nav">
    ...
    // Navigation handlers
    modal.querySelector('.prev-btn').addEventListener('click', () => navigateScreenshot(-1, modal));
    modal.querySelector('.next-btn').addEventListener('click', () => navigateScreenshot(1, modal));
    
    // Keyboard navigation
    document.addEventListener('keydown', (e) => {
        if (modal.style.display !== 'none') {
            if (e.key === 'ArrowLeft') navigateScreenshot(-1, modal);
            if (e.key === 'ArrowRight') navigateScreenshot(1, modal);
...
function navigateScreenshot(direction, modal) {
    if (!modal) {
        modal = document.getElementById('screenshotModal');
    }
    if (!modal || !currentApp.screenshots) return;
    
    const newIndex = (modal.currentIndex || 0) + direction;
